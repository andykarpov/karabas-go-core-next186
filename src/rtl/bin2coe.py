...
for i in range(0,len(data), 4):
    filecoe.write( tohex(data[i+3])+tohex(datao+[i+2])+  tohex(data[i+1]])+  tohex(data[i]) + ',\n')

filecoe.write( tohex(data[-1])+tohex(datao+[-2])+  tohex(data[-3]])+  tohex(data[-4]) + '\n')
